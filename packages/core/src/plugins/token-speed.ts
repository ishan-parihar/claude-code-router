...
export function getTokenSpeedStats(requestId: string): TokenStats | undefined {
  return requestStats.get(requestId);
}

/**
 * Get global token speed statistics (average of active requests)
 */
export function getGlobalTokenSpeedStats(): { activeRequests: number; averageTokensPerSecond: number } {
  if (requestStats.size === 0) {
    return { activeRequests: 0, averageTokensPerSecond: 0 };
  }
  ...
  for (const stats of requestStats.values()) {
    if (stats.tokensPerSecond > 0) {
      totalSpeed += stats.tokensPerSecond;
      count++;
    }
  }

  return {
    activeRequests: requestStats.size,
    averageTokensPerSecond: count > 0 ? Math.round(totalSpeed / count) : 0
  };
}
