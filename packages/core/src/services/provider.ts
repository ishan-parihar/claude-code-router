...
export class ProviderService {
  ...
  /**
   * Auto-detect provider type from api_base_url when type is not explicitly set.
   * This ensures that provider variants (e.g. iflowA, iflowX) are correctly
   * identified as their base type for headers, signing, and error handling.
   */
  private detectProviderType(apiBaseUrl: string): string | undefined {
    const url = apiBaseUrl.toLowerCase();
    if (url.includes('iflow.cn')) {
      return 'iflow';
    }
