...
import { CONFIG_FILE, HOME_DIR, listPresets } from "@ishan-parihar/shared";
...
import { pluginManager, tokenSpeedPlugin } from "@ishan-parihar/llms";
    ...
    if (req.sessionId && req.pathname?.endsWith("/v1/messages")) {
      if (payload instanceof ReadableStream) {
        if (req.agents) {
          ...
          // Track connection state for heartbeat management
          ...
          reply.raw.on('error', (err: any) => {
            console.error('[Agent Stream] Connection error:', err);
            isConnected = false;
            abortController.abort();
          });

          // Store Anthropic format message body, distinguishing text and tool types
          const processor = async (data: any, controller: ReadableStreamController<any>) => {
            try {
                ...
                try {
                  ...
                  try {
                    while (isConnected) {
                      ...
                      // Scenario-aware read timeout (10 min for reasoning models)
                      const scenarioTimeouts: Record<string, number> = {
                        think: 600000,      // 10 minutes for reasoning models
                        default: 180000,    // 3 minutes for default
                        longContext: 300000, // 5 minutes for long context
                        background: 120000,  // 2 minutes for background tasks
                        webSearch: 120000,   // 2 minutes for web search
                      };
                      const readTimeoutMs = scenarioTimeouts[req.scenarioType] || 60000;
                      ...
                      try {
                        const {value, done} = await Promise.race([
                          ...
                          new Promise<never>((_, reject) => {
                            timeoutId = setTimeout(() => reject(new Error('Read timeout')), readTimeoutMs);
                          })
                        ]);
                        ...
                        // Better backpressure handling with timeout
                        if (controller.desiredSize !== null && controller.desiredSize <= 0) {
                          // Wait for buffer to drain with timeout
                          const drainTimeout = setTimeout(() => {
                            console.warn('[Agent Stream] Backpressure timeout, dropping data');
                          }, 10000);

                          while (controller.desiredSize !== null && controller.desiredSize <= 0 && isConnected) {
                    ...
                    if (readError.name === 'AbortError' || readError.code === 'ERR_STREAM_PREMATURE_CLOSE' || readError.message === 'Read timeout') {
                      abortController.abort();
                      console.log('[Agent Stream] Stream read aborted or timed out');
                    } else {
                      throw readError;
                    }
                  } finally {
                  ...
                  console.error('[Agent Stream] Fetch error during tool processing:', fetchError);
                }
              ...
              console.error('[Agent Stream] Unexpected error in stream processing:', error);

              // Handle premature stream closure error
              if (error.code === 'ERR_STREAM_PREMATURE_CLOSE' || error.message === 'Client disconnected') {
                abortController.abort();
                return undefined;
              }
          ...
          const resultStream = rewriteStream(eventStream, processor)
            .pipeThrough(new SSESerializerTransform())
            .pipeThrough(new HeartbeatInjectorTransform(30000));

          // Clean up heartbeat via passthrough TransformStream (avoids locking)
          const { readable, writable } = new TransformStream({});
          resultStream.pipeTo(writable).catch((err) => {
            console.error('[Agent Stream] Pipeline error:', err);
          });
...
export { pluginManager, tokenSpeedPlugin } from "@ishan-parihar/llms";
